...
instr_Pl['depth_p'] = np.array([3.5, 4.0, 5.0, 1.96, 1.17, 1.75, 7.31])*60.0  # in muK - arcmin
instr_Pl['frequency'] = np.array([30, 44, 70, 100, 143, 217, 353])

if SAC_yrs_LF != 0:
    frequencies_plot = np.array([27,  39,  93, 145, 225, 280])
else:
    frequencies_plot = np.array([93, 145, 225, 280])  # 2 first freq removed as LF not in place yet
frequencies_plot = np.append(frequencies_plot, instr_Pl['frequency'])
freq_number = frequencies_plot.shape[0]
